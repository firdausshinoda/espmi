...
class Print_Laporan_Pengguna_Biasa extends FPDF {
	protected $B = 0;
	protected $I = 0;
	protected $U = 0;
	protected $HREF = '';
	...
	function isi($data){
		...
		$this->Cell(1);
		$this->SetFont('Times','','12');
		$this->cell(0,5,"Perihal            : Laporan Daftar Pengguna Biasa",0,1);
		...
		$this->Cell(65,8,"Nama",1,0,'C',true);
		$this->Cell(65,8,"NIPY",1,0,'C',true);
		$this->Cell(45,8,"Tanggal Dibuat",1,0,'C',true);

		$this->Ln();
		$no=1;
		$this->setFont('times','',12);
		if ($data->num_rows() > 0){
			$this->SetWidths(array(15,65,65,45));$this->SetAligns(array('C','L','L','L'));
			foreach ($data->result() as $it){
				$this->Row(array($no++,$it->nama_user,$it->nipy,setTglIndo($it->cdate)));
			}
		} else {
