...
class Print_Laporan_Unit extends FPDF {
	protected $B = 0;
	protected $I = 0;
	protected $U = 0;
	protected $HREF = '';
	...
	function isi($data){
		...
		$this->Cell(1);
		$this->SetFont('Times','','12');
		$this->cell(0,5,"Perihal            : Laporan Daftar Unit",0,1);
		...
		$this->Cell(65,8,"Nama Unit",1,0,'C',true);
		$this->Cell(65,8,"Keterangan",1,0,'C',true);
		...
		$this->setFont('times','',12);
		if ($data->num_rows() > 0){
			$this->SetWidths(array(15,65,65,45));$this->SetAligns(array('C','C','C','C'));
			foreach ($data->result() as $it){
				$this->Row(array($no++,$it->nama_unit,$it->keterangan,setTglIndo($it->cdate)));
			}
		} else {
