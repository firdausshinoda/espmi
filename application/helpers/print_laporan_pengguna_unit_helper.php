...
class Print_Laporan_Pengguna_Unit extends FPDF {
	protected $B = 0;
	protected $I = 0;
	protected $U = 0;
	protected $HREF = '';
	...
	function isi($data){
		...
		$this->Cell(1);
		$this->SetFont('Times','','12');
		$this->cell(0,5,"Perihal            : Laporan Daftar Pengguna Unit",0,1);
		...
		$this->Cell(40,8,"Nama",1,0,'C',true);
		$this->Cell(35,8,"NIPY",1,0,'C',true);
		$this->Cell(30,8,"Unit",1,0,'C',true);
		$this->Cell(30,8,"Jenis Unit",1,0,'C',true);
		...
		$this->setFont('times','',12);
		if ($data->num_rows() > 0){
			$this->SetWidths(array(15,40,35,30,30,40));$this->SetAligns(array('C','L','L','L','L','L'));
			foreach ($data->result() as $it){
				$this->Row(array($no++,$it->nama_admin,$it->nipy,$it->nama_unit,$it->jenis_unit,setTglIndo($it->cdate)));
			}
		} else {
