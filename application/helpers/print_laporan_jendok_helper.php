...
class Print_Laporan_Jendok extends FPDF {
	protected $B = 0;
	protected $I = 0;
	protected $U = 0;
	protected $HREF = '';
	...
	function isi($data){
		...
		$this->Cell(1);
		$this->SetFont('Times','','12');
		$this->cell(0,5,"Perihal            : Laporan Daftar Jenis Dokumen",0,1);
		...
		$this->Cell(60,8,"Jenis Dokumen",1,0,'C',true);
		$this->Cell(60,8,"Keterangan",1,0,'C',true);
		$this->Cell(50,8,"Tanggal Dibuat",1,0,'C',true);

		$this->Ln();
		$no=1;
		$this->setFont('times','',12);
		if ($data->num_rows() > 0){
			$this->SetWidths(array(20,60,60,50));$this->SetAligns(array('C','L','L','L'));
			foreach ($data->result() as $it){
				$this->Row(array($no++,$it->jenis_dokumen,$it->keterangan,setTglIndo($it->cdate)));
			}
		} else {
