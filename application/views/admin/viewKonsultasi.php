		...
		<div class="col-sm-12 col-md-6">
			<div class="card shadow">
				<div class="card-body">
					...
					<table>
						...
						<tr>
							<?php if ($item->stt_dokumen=="MENUNGGU"):?>
								<td><small>Tanggal Unggah</small></td><td>&nbsp;:&nbsp;</td><td><small><?= waktu_lalu($item->cdate); ?></small></td>
							<?php else:?>
								<td><small>Tanggal Publis</small></td><td>&nbsp;:&nbsp;</td><td><small><?= waktu_lalu($item->publis_date); ?></small></td>
							<?php endif; ?>
						</tr>
						<tr>
					...
					<hr>
					<a href="<?= base_url('Admin/konsultasi?page=detail&id='.$item->id_tb_konsultasi); ?>" class="btn btn-success btn-sm text-white float-right m-1"><i class="fa fa-eye"></i> Lihat</a>
					<?php if ($item->id_tb_admin == sess_get('id')): ?>
						<?php if ($item->stt_dokumen!="PUBLIS"):?>
							<a href="<?= base_url('Admin/konsultasi?page=edit&id='.$item->id_tb_konsultasi); ?>" class="btn btn-warning btn-sm text-white float-right m-1"><i class="fa fa-pencil-alt"></i> Edit</a>
							<button class="btn btn-danger btn-sm text-white float-right m-1" onclick="hapus('hapus-konsultasi','<?= $item->id_tb_konsultasi; ?>')"><i class="fa fa-trash-alt"></i> Hapus</button>
						<?php endif; ?>
					<?php endif; ?>
				</div>
			</div>
