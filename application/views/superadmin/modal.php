		...
		<div class="modal-content">
			<form id="formExecuteModal" class="formExecuteModal">
				<div class="modal-header">
					<h6 class="modal-title" id="exampleModalLabel">Tambah Revisi</h6>
				...
				<div class="modal-body">
					<input type="hidden" id="id_konsultasi_unit_terkait" name="id_konsultasi_unit_terkait" value="<?= $id_konsultasi_unit_terkait; ?>">
					<div class="form-group mb-0">
						<label for="keterangan" class="col-form-label"><small>Keterangan</small></label>
						<div id="keterangan" name="keterangan"></div>
					</div>
				</div>
...
<script type="text/javascript">
    ...
    CKEDITOR.config.removeButtons = 'Image,Table,Flash,Iframe';
    ...
    $(function() {
        $('#formExecuteModal').validate({
            ...
            submitHandler: function(form) {
				...
				data.append("id_konsultasi", getUrlVars()["id"]);
				data.append("id_konsultasi_unit_terkait", $('#id_konsultasi_unit_terkait').val());
				data.append(csrfName, csrfHash);
				data.append("keterangan", CKEDITOR.instances.keterangan.getData());
                $.ajax({
                    type: "POST", data:data, url: "<?= site_url('Api/api_inputRevisi'); ?>",
                    ...
                    success: function(data) {
                        ...
                        if (data.response.status === "OK"){
                            swalWithBootstrapButtons.fire({title: 'Berhasil Menambahkan Revisi.', text: "", icon: 'success',}).
                            then((result) => {if (result.value) {location.reload();}});
                        } else {
                            alertError('Gagal Menambahkan Revisi');
                        }
                    },
                    error:function(data){
                        $('#progress-modal').css('visibility', 'hidden');
