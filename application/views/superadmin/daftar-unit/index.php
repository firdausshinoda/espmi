			...
			<div class="col-sm-12 mt-3">
				<div class="card card-white">
					<div class="card-header">
						<h3 class="card-title"><b>Daftar Unit</b></h3>
					</div>
					<div class="card-body">
...
<a href="<?= site_url('Superadmin/dafunit?page=new'); ?>" class="btn btn-primary btn-lg btn-floating shadow rounded-circle">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        dt = $("#dt_table").DataTable({
            "dom": 'Blfrtip',
            "buttons" : [
                { extend: 'csv', messageTop: 'Daftar Unit',
                    exportOptions: { columns: [ 0, 1, 2 ,3 ] },
                    title: 'Daftar Unit',
                },
                {
                    extend: 'excel', messageTop: 'Daftar Unit',
                    exportOptions: { columns: [ 0, 1, 2 ,3 ] },
                    title: 'Daftar Unit',
                },
                {
                    extend: 'pdf', messageTop: 'Daftar Unit', messageBottom: null,
                    exportOptions: { columns: [ 0, 1, 2 ,3 ] },
                    title: 'Daftar Unit',
                },
            ],
            ...
            "paging": true,
            "ajax": {
                "url": '<?= site_url('Api/api_getDafUnit');?>',
                "type": "POST",
                "data": function (d) {
            ...
            "columns": [
                ...
                {"render": function ( data, type, row ) {
                        return '<a href="<?= site_url('Superadmin/dafunit?page=detail&id=')?>'+row.id_tb_unit+'">'+row.nama_unit+'</a>';
                    }
                },
                {"data": "keterangan"},
                {"render": function ( data, type, row ) {
                ...
                {"render": function ( data, type, row ) {
                        return '<a href="<?= site_url('Superadmin/dafunit?page=edit&id='); ?>'+row.id_tb_unit+'" class="btn btn-warning btn-sm btn-block shadow-sm">Edit</a>\n' +
                            '<button type="button" class="btn btn-danger btn-sm btn-block shadow-sm" onclick="hapus(\'dafunit\',\''+row.id_tb_unit+'\')">Hapus</button>';
                    }
                },
            ],
